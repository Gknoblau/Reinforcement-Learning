...
for run in xrange(numRuns):
	...
	for episodeNum in xrange(numEpisodes):
		...
		position, velocity = state # Gets the initial position and velocity
		E = numpy.zeros(n)
		step = 0
		while state != None:
			...
			if nextState == None:
				...
				action1 = pickAction(theta,Q,epsilon)
				value1 = 0
				for i in F:
					value1 = value1 + theta[i+action1*323]

				tilecode(position, velocity, F)
				value2 = 0
				for i in F:
					value2 = value2 + theta[i+action*323]

				delta = reward + (value1 - value2)
	...
	#if (run == numRuns/2):
		#Epi = Emu = epsilon = 0.01 # Worked
		#Epi = Emu = epsilon = 0.9 # Horrible! overflow
		#Epi = Emu = epsilon = 0.09 # Got stuck, overflow
	#	Epi = Emu = epsilon = 0.01 # 50 runs, 200 episodes, mean normal = , mean Changed = , mean Std Normal = , mean Std Changed =
	# Default Epsilon
	#if (epsilon == 0):
